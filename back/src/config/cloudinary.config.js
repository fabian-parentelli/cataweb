...
const deleteRawFile = async (url) => {
    try {
        const publicId = getPublicId(url);
        const result = await cloudinary.uploader.destroy(publicId, { resource_type: 'raw' });
        return result;
    } catch (error) {
        console.error("Error al eliminar el archivo raw de Cloudinary:", error);
        throw new Error('No se pudo eliminar el archivo raw');
    };
};

export { uploadToCloudinary, deleteImg, getPublicId, deleteRawFile };